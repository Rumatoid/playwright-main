...
export class CRNetworkManager {
  ...
  constructor(
    ...
    parentManager: CRNetworkManager | null
  ) {
    this._client = client;
    this._page = page;
  ...
  instrumentNetworkEvents(
    ...
    const listeners = [
      ...
      eventsHelper.addEventListener(
        ...
        this._onLoadingFailed.bind(this, workerFrame)
      ),
    ];
    if (this._page) {
      listeners.push(
  ...
  _onRequestWillBeSent(
    workerFrame: frames.Frame | undefined,
  ...
  _onRequest(
    ...
    if (
      ...
      requestWillBeSentEvent.frameId ===
        (this._page?._delegate as CRPage)._targetId
    ) {
    ...
    if (
      ...
      !requestPausedEvent.responseErrorReason
    ) {
      ...
      request.request.setRawRequestHeaders(
        headersObjectToArray(requestPausedEvent.request.headers, "\n")
      );
    }
  ...
  _createResponse(
    ...
    const getResponseBody = async () => {
      ...
      const expectedLength = contentLengthHeader
    ...
    if (
      timingPayload &&
      !this._responseExtraInfoTracker.servedFromCache(request._requestId)
    ) {
      timing = {
        startTime:
    ...
    const response = new network.Response(
      ...
      headersObjectToArray(responsePayload.headers),
      timing,
      getResponseBody,
    ...
    if (
      ...
      typeof responsePayload?.remotePort === "number"
    ) {
  ...
  _handleRequestRedirect(
    ...
    (this._page?._frameManager || this._serviceWorker)!.requestReceivedResponse(
      response
    );
    (this._page?._frameManager || this._serviceWorker)!.reportRequestFinished(
      request.request,
      response
    );
  }
  ...
  _onResponseReceivedExtraInfo(
    event: Protocol.Network.responseReceivedExtraInfoPayload
  ) {
  ...
  _onResponseReceived(event: Protocol.Network.responseReceivedPayload) {
    ...
    if (
      ...
      const requestWillBeSentEvent =
        this._requestIdToRequestWillBeSentEvent.get(event.requestId);
      const frame = requestWillBeSentEvent?.frameId
        ? this._page?._frameManager.frame(requestWillBeSentEvent.frameId)
        : null;
  ...
  _onLoadingFinished(event: Protocol.Network.loadingFinishedPayload) {
    ...
    if (response) {
      ...
      response
        .responseHeadersSize()
        .then((size) =>
          response.setEncodedBodySize(event.encodedDataLength - size)
        );
      response._requestFinished(
        helper.secondsToRoundishMillis(event.timestamp - request._timestamp)
      );
    }
    this._requestIdToRequest.delete(request._requestId);
    if (request._interceptionId)
  ...
  _onLoadingFailed(
    workerFrame: frames.Frame | undefined,
    event: Protocol.Network.loadingFailedPayload
  ) {
  ...
  private _maybeAdoptMainRequest(
    requestId: Protocol.Network.RequestId
  ): InterceptableRequest | undefined {
...
function splitSetCookieHeader(headers: types.HeadersArray): types.HeadersArray {
  ...
  result.splice(
    index,
    1,
    ...values.map((value) => ({ name: header.name, value }))
  );
  return result;
}
...
type RequestInfo = {
  ...
  requestWillBeSentExtraInfo: (
    | Protocol.Network.requestWillBeSentExtraInfoPayload
    | undefined
  )[];
  responseReceivedExtraInfo: (
    | Protocol.Network.responseReceivedExtraInfoPayload
    | undefined
  )[];
  ...
  private _patchHeaders(info: RequestInfo, index: number) {
    ...
    if (response && requestExtraInfo) {
      response
        ...
        .setRawRequestHeaders(
          headersObjectToArray(requestExtraInfo.headers, "\n")
        );
    ...
    if (response && responseExtraInfo) {
      response.setResponseHeadersSize(
        responseExtraInfo.headersText?.length || 0
      );
