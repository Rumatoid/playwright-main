...
export class Chromium extends BrowserType {
  ...
  override async connectOverCDP(
    metadata: CallMetadata,
    endpointURL: string,
    options: { slowMo?: number; headers?: types.HeadersArray },
    timeout?: number
  ) {
  ...
  async _connectOverCDPInternal(
    ...
    if (!headersMap) headersMap = { "User-Agent": getUserAgent() };
    else if (
      headersMap &&
    ...
    const persistent: channels.BrowserNewContextParams = {
  ...
  private _createDevTools() {
    ...
    return directory
      ? new CRDevTools(path.join(directory, "devtools-preferences.json"))
      : undefined;
  }

  async _connectToTransport(
  ...
  override async _launchWithSeleniumHub(
    progress: Progress,
    hubUrl: string,
    ...
    try {
      if (process.env.SELENIUM_REMOTE_CAPABILITIES) {
        ...
        progress.log(
          `<selenium> using additional capabilities "${process.env.SELENIUM_REMOTE_CAPABILITIES}"`
        );
      }
    } catch (e) {
      progress.log(
        `<selenium> ignoring additional capabilities "${process.env.SELENIUM_REMOTE_CAPABILITIES}": ${e}`
      );
    }
    ...
    try {
      ...
      if (capabilities["se:cdp"]) {
        ...
        const chromeOptionsURL =
          typeof maybeChromeOptions === "string"
            ? maybeChromeOptions
            : undefined;
        // TODO(dgozman): figure out if we can make ChromeDriver to return 127.0.0.1 instead of localhost.
        const endpointURLString = addProtocol(
          debuggerAddress || chromeOptionsURL
